...
using SparseArrays, Pkg, MAT, CSV, DataFrames, Distances, SparseArrays, Random, Plots, Gurobi, JuMP, NPZ, Statistics, JSON
...
function Max_Coverage_Kernel_Masked_Budget_StationMax_UniformFixedDrones(
    static_map_file, budget_millions, cost_sensor_millions, cost_station_millions, cost_drone_millions,
    kernel, kernel_size_x, kernel_size_y, mask_file, recompute_kernel=false, n_steps=63,
    time_limit_seconds=600.0, fixed_drones_per_station::Int=7, candidate_percentile=0.80, warm_start_file="",
    budget_regularization_epsilon=-1.0
)
    """
    Budget-constrained StationMax placement: each open charging station has exactly
    `fixed_drones_per_station` drones (default 7).  Coverage per (cell, station) is
    binary: if the cell is assigned to an open station within one-way reach, that
    station contributes 1 to the cell's theta (no fractional uniform kernel, no w).

    Budget per open station: cost_station + fixed_drones_per_station * cost_drone.

    Optional warm_start_file: same JSON as greedy-uniform (ground_sensor_locations,
    charging_station_locations, drones_per_charging_station) — sets MIP start for
    xg and y only (assignment u is left to the solver).
    """
    ...
    station_unit_cost = cost_station_millions + fixed_drones_per_station * cost_drone_millions

    println("=== Max_Coverage_Kernel_Masked_Budget_StationMax_UniformFixedDrones ===")
    println("static_map_file=", static_map_file)
    println("budget=", budget_millions, "M")
    println("costs: sensor=", cost_sensor_millions, "M, station+",
            fixed_drones_per_station, " drones=", station_unit_cost, "M per open station")
    println("fixed_drones_per_station=", fixed_drones_per_station)
    println("candidate_percentile=", candidate_percentile)
    if budget_regularization_epsilon >= 0.0
        epsilon = budget_regularization_epsilon
    else
        epsilon = budget_millions > 300.0 ? 0.1 : 0.0
    end
    println("budget regularization epsilon=", epsilon)
    ...
    max_possible_grounds   = floor(Int, budget_millions / cost_sensor_millions)
    max_possible_charging  = floor(Int, budget_millions / station_unit_cost)
    println("Max possible: sensors=", max_possible_grounds,
            ", charging stations (each with ", fixed_drones_per_station, " drones)=", max_possible_charging)
    ...
    for station in I_second
        local_pairs = Tuple{Tuple{Int,Int}, Tuple{Int,Int}}[]
        reachable_cells = reachable_masked_cells_within_reach(station, mask, N, M, one_way_reach)
        for cell in reachable_cells
            pair = (cell, station)
            push!(feasible_pairs, pair)
            push!(local_pairs, pair)
    ...
    G = length(I_prime)
    S = length(I_second)
    P = length(feasible_pairs)
    C = length(I)
    println("MIP size (UniformFixedDrones): binary xg=", G, ", y=", S, ", u=", P,
            " (total bin ", G + S + P, "); continuous theta=", C, " (no w)")
    ...
    xg    = @variable(model, [i in I_prime], Bin)
    y     = @variable(model, [s in I_second], Bin)
    u     = @variable(model, [pair in feasible_pairs], Bin)
    theta = @variable(model, [i in I])

    budget_used_expr = @expression(model,
        cost_sensor_millions * sum(xg) + station_unit_cost * sum(y)
    )
    ...
    @constraint(model, [i in I_common], xg[i] + y[i] <= 1)
    @constraint(model, [pair in feasible_pairs], u[pair] <= y[pair[2]])
    @constraint(model, [cell in keys(cell_to_pairs)],
        sum(u[pair] for pair in cell_to_pairs[cell]) <= 1)

    @constraint(model, [i in I], 0 <= theta[i] <= 1)
    @constraint(model, [cell in I; cell in I_prime_set],
        theta[cell] <= xg[cell] + sum(u[pair] for pair in get(cell_to_pairs, cell, Tuple{Tuple{Int,Int}, Tuple{Int,Int}}[])))
    @constraint(model, [cell in I; !(cell in I_prime_set)],
        theta[cell] <= sum(u[pair] for pair in get(cell_to_pairs, cell, Tuple{Tuple{Int,Int}, Tuple{Int,Int}}[])))

    time_model_creation_end = time_ns() / 1e9
    println("Model creation took ", round(time_model_creation_end - time_model_creation_start, digits=2), " seconds")
    flush(stdout)

    if warm_start_file != "" && isfile(warm_start_file)
        println("\n=== WARM START (UniformFixedDrones) from ", warm_start_file, " ===")
        ws_data = JSON.parsefile(warm_start_file)

        ws_ground = Set{Tuple{Int,Int}}()
        for loc in get(ws_data, "ground_sensor_locations", [])
            push!(ws_ground, (Int(loc[1]) + 1, Int(loc[2]) + 1))
        end

        ws_stations = Set{Tuple{Int,Int}}()
        for loc in get(ws_data, "charging_station_locations", [])
            push!(ws_stations, (Int(loc[1]) + 1, Int(loc[2]) + 1))
        end

        n_xg = 0
        for i in I_prime
            if i in ws_ground
                set_start_value(xg[i], 1.0)
                n_xg += 1
            else
                set_start_value(xg[i], 0.0)
            end
        end
        ...
        for s in I_second
            if s in ws_stations
                set_start_value(y[s], 1.0)
                n_y += 1
            else
                set_start_value(y[s], 0.0)
            end
        end

        println("  MIP start: xg=1 on ", n_xg, " / ", length(ws_ground), " ground locs (in I_prime)")
        println("  MIP start: y=1 on ", n_y, " / ", length(ws_stations), " station locs (in I_second)")
        println("=== END WARM START ===\n")
        flush(stdout)
    elseif warm_start_file != ""
        println("WARNING: warm_start_file '", warm_start_file, "' not found — solving from scratch.")
        flush(stdout)
    end
    ...
    selected_x_indices = [(i[1] - 1, i[2] - 1) for i in I_prime if value(xg[i]) > 0.5]
    selected_y_indices = Tuple{Int,Int}[]
    drone_allocations = Int[]
    for station in I_second
        if value(y[station]) > 0.5
            push!(selected_y_indices, (station[1] - 1, station[2] - 1))
            push!(drone_allocations, fixed_drones_per_station)
        end
    end
    ...
    n_stations = length(selected_y_indices)
    n_drones = sum(drone_allocations; init=0)
    budget_used = n_sensors * cost_sensor_millions + n_stations * station_unit_cost

    println("\n=== BUDGET ALLOCATION ===")
    println("  Ground sensors:     ", n_sensors, " (cost ", round(n_sensors * cost_sensor_millions, digits=2), "M)")
    println("  Charging stations:  ", n_stations, " (cost ", round(n_stations * station_unit_cost, digits=2), "M incl. drones)")
    println("  Drones:             ", n_drones, " (", fixed_drones_per_station, " per station)")
    println("  Budget used:        ", round(budget_used, digits=2), "M / ", budget_millions, "M")
...
function Max_Coverage_Kernel_Masked_Budget_StationMax_GreedyUniform(static_map_file, budget_millions, cost_sensor_millions, cost_station_millions, cost_drone_millions, kernel, kernel_size_x, kernel_size_y, mask_file, recompute_kernel=false, n_steps=63, time_limit_seconds=600.0, max_drones_per_station=7, candidate_percentile=0.80, warm_start_file="", fixed_drones_per_station=0)
    """
    ...
    println("max_drones_per_station=", max_drones_per_station)
    println("candidate_percentile=", candidate_percentile)
    if fixed_drones_per_station > 0
        println("fixed_drones_per_station=", fixed_drones_per_station, " (each open station has exactly this many drones)")
    end
    epsilon = budget_millions > 300.0 ? 0.1 : 0.0
    println("budget regularization epsilon=", epsilon)
    ...
    if fixed_drones_per_station > 0
        fixed = fixed_drones_per_station
        for station in I_second
            cap = station_drone_caps[station]
            if fixed > cap
                error("fixed_drones_per_station (", fixed, ") exceeds per-station cap ", cap, " at station ", station)
            end
        end
        @constraint(model, [station in I_second],
            sum(z[(station, level)] for level in 1:station_drone_caps[station]) == fixed * z[(station, 1)])
    end
    ...
    # ---- MIP warm start from a previous solution JSON ----
    if warm_start_file != "" && isfile(warm_start_file)
        println("\n=== WARM START from ", warm_start_file, " ===")
        ws_data = JSON.parsefile(warm_start_file)
        ...
        ws_stations = Tuple{Int,Int}[]
        ws_drones   = Int[]
        for (idx, loc) in enumerate(get(ws_data, "charging_station_locations", []))
            alloc = get(ws_data, "drones_per_charging_station", Int[])
            nd = idx <= length(alloc) ? Int(alloc[idx]) : 0
            push!(ws_stations, (Int(loc[1]) + 1, Int(loc[2]) + 1))
            push!(ws_drones, nd)
        end
        ws_station_drones = Dict(zip(ws_stations, ws_drones))
        ...
        for station in I_second
            cap = station_drone_caps[station]
            nd = get(ws_station_drones, station, 0)
            for level in 1:cap
                if nd >= level
                    set_start_value(z[(station, level)], 1.0)
                    n_z_set += 1
                else
                    set_start_value(z[(station, level)], 0.0)
                end
            end
        end

        println("  Warm-start sensors set:  ", n_xg_set, " / ", length(ws_ground), " from file")
        println("  Warm-start station-drone levels set: ", n_z_set)
        println("=== END WARM START ===\n")
        flush(stdout)
    elseif warm_start_file != ""
        println("WARNING: warm_start_file '", warm_start_file, "' not found — solving from scratch.")
...
# ---- end of Max_Coverage_Kernel_Masked_Budget_StationMax_GreedyUniform ----
