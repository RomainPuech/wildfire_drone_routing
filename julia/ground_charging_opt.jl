...
# ---------------------------------------------------------------------------------------------------------------------------------------
function NEW_ROUTING_STRATEGY_INIT(risk_pertime,n_drones,ChargingStations,GroundStations,optimization_horizon,max_battery_time,reevaluation_step) #T here is optimization_horizon: should be > max_battery_time, reevaluation_step < floor(optimization_horizon/2)
...
function NEW_ROUTING_STRATEGY_NEXTMOVE(risk_pertime,n_drones,ChargingStations,GroundStations,optimization_horizon,max_battery_time,reevaluation_step,drone_locations,drone_states,battery_level) #T here is optimization_horizon: should be > max_battery_time, reevaluation_step < floor(optimization_horizon/2)
    ...
    for ((i,s), state) in drone_locations
        if state == "charge"
            @constraint(model, c[i,1,s] == 1)
        elseif state = "fly"
            @constraint(model, a[i,1,s] == 1)
        end
    end
    ...
    @constraint(model, [s in 1:n_drones], b[1,s] == battery_level[s]) #First run
