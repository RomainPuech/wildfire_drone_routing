...
function create_index_routing_model(risk_pertime_file, n_drones, ChargingStations, GroundStations, optimization_horizon, max_battery_time, objective_type="min_cumulative_prob")
    t1 = time_ns() / 1e9
    risk_pertime = load_burn_map(risk_pertime_file)
    _, N, M = size(risk_pertime)
    T = optimization_horizon
    ...
    if objective_type == "min_cumulative_prob"
        ...
        # zeta_it := last time I visited cell i (one for each drone) 
        # (t - 1) sum_{t'<= t-1}( x_it' ) >= z_it >= t' * x_it' , \forall t' <= t 
        zeta = @variable(model, [i=1:length(GridpointsDrones), t=1:T, s=1:n_drones], Int)
        @constraint(model, [t=1:T, i=1:length(GridpointsDrones), s=1:n_drones, t2=1:t], zeta[i,t,s] >= t2*a[i,t2,s]) # can be merged
        # @constraint(model, [t=1:T, i=1:length(GridpointsDrones), s=1:n_drones], zeta[i,t,s] <= t * sum(a[i,t2,s] for t2 in 1:t))
        # At most one drone per cell each t: sum_s(x_its) = 1, \forall i,t 
        @constraint(model, [i=1:length(GridpointsDrones), t=1:T], sum(a[i,t,s] for s in 1:n_drones) <= 1)
        @constraint(model, [t=1:T, k=1:length(ChargingStations), s=1:n_drones], a[grid_to_idx[ChargingStations[k]],t,s]==0)
    
        # w_itt2:= 1 if in t2 between last z_it visit and t // 0 otherwise 
        w = @variable(model, [i=1:length(GridpointsDrones), t=1:T, t2=1:t, s=1:n_drones], Bin)
   ...
   #MAKES IT VERY SLOW
    ...
    elseif objective_type == "min_cumulative_prob"
        @objective(model, Min, sum(w[i,t,t2,s]*risk_idx[t,i] for t=1:T, t2=1:t, i=1:length(GridpointsDrones), s=1:n_drones))
    end
    # Initialize constraint containers
    init_constraints = ConstraintRef[]
    next_move_constraints = ConstraintRef[]
