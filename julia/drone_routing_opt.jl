...
function create_regularized_index_routing_model(risk_pertime_file, n_drones, ChargingStations, GroundStations, optimization_horizon, max_battery_time, regularization_param)
    ...
    println(ChargingStations)
    return RegularizedIndexRoutingModel(model, a, c, b, theta, total_linf_dist, init_constraints, next_move_constraints, 
                        GridpointsDrones, ChargingStations, GridpointsDronesDetecting, risk_pertime, T, n_drones, grid_to_idx, charging_map, max_battery_time, regularization_param)
end
...
function solve_regularized_index_next_move_routing(routing_model::RegularizedIndexRoutingModel, reevaluation_step, drone_locations, drone_states, battery_level, offset=0)
    model = routing_model.model
    a = routing_model.a
    c = routing_model.c
    b = routing_model.b
    ...
    T = routing_model.T
    n_drones = routing_model.n_drones
    risk_pertime = routing_model.risk_pertime
    GridpointsDronesDetecting = routing_model.GridpointsDronesDetecting
    regularization_param = routing_model.regularization_param
    ...
    H = size(risk_pertime, 1)
    if T + offset > H
        error("Offset too large: T + offset = $(T + offset) exceeds risk_pertime time horizon H = $H")
    end
    
    # Get the total_linf_dist expression from the model
    total_linf_dist = model.total_linf_dist
    
    @objective(model, Max, 
        sum([risk_pertime[1+offset, GridpointsDronesDetecting[k]...]*(theta[1,k]) for k in 1:length(GridpointsDronesDetecting)]) + 
        sum(risk_pertime[t+offset, GridpointsDronesDetecting[k]...]*(theta[t,k] - theta[t-1,k]) for t in 2:T, k in 1:length(GridpointsDronesDetecting)) + 
        regularization_param * total_linf_dist
    )
...
# ----------- NEW FORMULATION ---------------- #
struct NewIndexRoutingModel
    model::Model
    a::AbstractArray{VariableRef, 3}
    c::AbstractArray{VariableRef, 3}
    b::AbstractArray{VariableRef, 2} # abstract array keeps the axes (custom indexing)
    theta::AbstractArray{VariableRef, 2}
    init_constraints::Vector{ConstraintRef}
    next_move_constraints::Vector{ConstraintRef}
    GridpointsDrones::Vector{Tuple{Int,Int}}
    ChargingStations::Vector{Tuple{Int,Int}}
...
function new_create_index_routing_model(risk_pertime_file, n_drones, ChargingStations, GroundStations, optimization_horizon, max_battery_time)
    max_battery_time = max_battery_time + 1
    println("Creating index routing model")
    t1 = time_ns() / 1e9
    ...
    a = @variable(model, [i=1:length(GridpointsDrones), t=1:T, s=1:n_drones], Bin) # Variable denoting if drone s flies at grid point i at time t
    c = @variable(model, [i=1:length(ChargingStations), t=1:T, s=1:n_drones], Bin) # Variable denoting if drnoe s charges at grid point i at time t
    b = @variable(model, [t=0:T, s=1:n_drones], Int) # Variable denoting the battery of drone s at time t, defined as the # of time steps drone s can operate without recharging
    # println("axes(b): ", axes(b))
    theta = @variable(model, [t=1:T, k=1:length(GridpointsDronesDetecting)], Bin) # Variable denoting if grid point k is covered by a drone at time t  
    ...
    # Min/max battery level constraints
    @constraint(model, [t=1:T, s=1:n_drones], 1 <= b[t,s] <= max_battery_time)
    @constraint(model, [s in 1:n_drones], b[0,s] == max_battery_time)
    
    # Battery dynamics
    @constraint(model, [s in 1:n_drones, t in 1:T], b[t,s] >= max_battery_time*sum(c[i,t,s] for i in 1:length(ChargingStations)))
    @constraint(model, [t in 0:T-1, s in 1:n_drones], 
        b[t+1,s] <= b[t,s] - 1 + max_battery_time * sum(c[i,t+1,s] for i in 1:length(ChargingStations)))
...
######
    ...
    println("Model created in ", t2 - t1, " seconds")
    println(ChargingStations)
    return NewIndexRoutingModel(model, a, c, b, theta, init_constraints, next_move_constraints, 
                        GridpointsDrones, ChargingStations, GridpointsDronesDetecting, risk_pertime, T, n_drones, grid_to_idx, charging_map, max_battery_time)
end

function new_solve_index_init_routing(routing_model::NewIndexRoutingModel, reevaluation_step)
    # println("Solving index init routing")
    model = routing_model.model
    a = routing_model.a
    c = routing_model.c
    b = routing_model.b
    # println("in new_solve_index_init_routing axes(b): ", axes(b))
    ChargingStations = routing_model.ChargingStations
    GridpointsDrones = routing_model.GridpointsDrones
    grid_to_idx = routing_model.grid_to_idx
    T = routing_model.T
    ...
    sb_constraint = @constraint(model, [s=1:n_drones-1, i=1:length(GridpointsDrones), j=i+1:length(GridpointsDrones)], a[i,1,s] <= a[j,1,s+1])
    append!(routing_model.init_constraints, values(sb_constraint))
