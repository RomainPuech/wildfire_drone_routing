...
# Mock the problematic imports before importing Strategy
def mock_save_burn_map(burnmap, filename):
    """Mock function to replace save_burn_map"""
    np.save(filename, burnmap)

def mock_load_scenario(filename):
    """Mock function to replace load_scenario"""  
    return np.load(filename)

# Patch the problematic modules
with patch.dict('sys.modules', {
    'rasterio': MagicMock(),
    'rasterio.transform': MagicMock(),
    'rasterio.crs': MagicMock(),
}):
    # Mock the dataset functions
    dataset_mock = MagicMock()
    dataset_mock.save_burn_map = mock_save_burn_map
    dataset_mock.load_scenario = mock_load_scenario
    
    with patch.dict('sys.modules', {'dataset': dataset_mock}):
...
def test_top_strategy_basic():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        auto_params = {
            ...
            "ground_sensor_locations": [],  # Empty list - will be converted to proper type in Strategy
