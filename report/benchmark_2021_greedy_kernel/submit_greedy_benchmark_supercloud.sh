...
PREPROCESS_JOB=$(sbatch --chdir="${PROJECT_ROOT}" "${SCRIPT_DIR}/slurm_preprocess_benchmark_2021_supercloud.sh" | awk '{print $4}')
echo "Submitted preprocessing job ${PREPROCESS_JOB}"

ARRAY_JOB=$(sbatch --chdir="${PROJECT_ROOT}" --dependency=afterok:${PREPROCESS_JOB} "${SCRIPT_DIR}/slurm_reproduce_greedy_kernel_supercloud.sh" | awk '{print $4}')
echo "Submitted array job ${ARRAY_JOB} (depends on preprocessing ${PREPROCESS_JOB})"

WRAPUP_JOB=$(sbatch --chdir="${PROJECT_ROOT}" --dependency=afterany:${ARRAY_JOB} "${SCRIPT_DIR}/slurm_reproduce_greedy_kernel_wrapup_supercloud.sh" | awk '{print $4}')
