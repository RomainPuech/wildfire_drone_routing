#!/usr/bin/env python3
"""
Quick test: run the station-max budget-based sensor placement on the California
2021 dataset with the same operational-scale preprocessing as the 2021
benchmark. Results are saved in the standard sensor_alloc cache format.
"""
...
from Strategy import SensorPlacementMaxCoverageGaussianTimeMaskedBudgetStationMax
...
def main(budget_millions: float = 20.0, time_limit_seconds: float = 600.0):
    ...
    combo_name = f"GaussianBudget{int(budget_millions)}M_StationMax"
    sensor_log_path = str(
    ...
    print("\n" + "=" * 70, flush=True)
    print("  Running SensorPlacementMaxCoverageGaussianTimeMaskedBudgetStationMax", flush=True)
    print("=" * 70 + "\n", flush=True)

    strat = SensorPlacementMaxCoverageGaussianTimeMaskedBudgetStationMax(auto_params, custom_params)
