#!/usr/bin/env python3
"""
Benchmark: Multiple strategy combinations on all scenarios from MiniTractDatasetFull.
...
DATASET_DIR = Path("MiniTractDatasetFull")
FIRES = sorted([d.name for d in DATASET_DIR.iterdir() if d.is_dir()])
print(f"Found {len(FIRES)} fire layouts: {FIRES}\n", flush=True)
...
for combo_idx, combo in enumerate(STRATEGY_COMBINATIONS, 1):
    ...
    for fire_name in FIRES:
        fire_dir = DATASET_DIR / fire_name
        scenario_dir = fire_dir / "scenarii"
        scenarios = sorted(scenario_dir.glob("*.npy"))
        
        print("-" * 70, flush=True)
        print(f"  FIRE: {fire_name}  ({len(scenarios)} scenario(s))", flush=True)
        print("-" * 70, flush=True)
        
        # --- Burn map: ensure it's 3D (T, N, M) ---
        burnmap_path = fire_dir / "burn_map.npy"
        burnmap_raw = np.load(str(burnmap_path))
        if burnmap_raw.ndim == 2:
            # Expand static 2D burn map to 3D with 1 time step
            burnmap_3d_path = fire_dir / "burn_map_3d.npy"
            if not burnmap_3d_path.exists():
                np.save(str(burnmap_3d_path), burnmap_raw[np.newaxis, ...])
                print(f"  Created 3D burn map: {burnmap_3d_path} (shape {burnmap_raw[np.newaxis].shape})", flush=True)
            burnmap_filename = str(burnmap_3d_path)
        else:
            burnmap_filename = str(burnmap_path)
        
        # --- Mask ---
        mask_path = fire_dir / "mask.npy"
        ...
        for scenario_file in scenarios:
            ...
            try:
                ...
                if tmp_files:
                    ...
                    out_name = f"benchmark_{combo_name}_{fire_name}_{scenario_name}"
                    
                    # Load mask (data scale) for gray overlay
                    mask_data = np.load(str(mask_path)) if mask_path.exists() else None
...
if all_results:
    ...
    metadata_cols = ["strategy_combo", "sensor_strategy", "drone_strategy", "fire_name", "scenario_name"]
    other_cols = [c for c in df.columns if c not in metadata_cols]
    df = df[metadata_cols + other_cols]
    
    # Compute averaged results for each strategy combo
    print("\nComputing averaged results per strategy combo...", flush=True)
    averaged_rows = []
    
    for combo_name in df["strategy_combo"].unique():
        combo_df = df[df["strategy_combo"] == combo_name].copy()
        ...
        avg_row = {
            ...
            "fire_name": "AVERAGE",
            "scenario_name": f"n={len(combo_df_clean)}",
        }
    ...
    # Group by strategy combo and fire, show key metrics
    if "delta_t" in df.columns:
        summary_cols = ["strategy_combo", "fire_name", "delta_t", "device"]
        ...
        if averaged_rows:
            print("\n" + "=" * 80, flush=True)
            print("  AVERAGED RESULTS BY STRATEGY COMBO", flush=True)
            print("=" * 80, flush=True)
