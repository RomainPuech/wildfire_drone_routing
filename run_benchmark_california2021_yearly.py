...
def build_strategy_combinations(budget_millions: float):
    """Two routing combos sharing the same Gaussian-budget sensor placement.

    1. TOP masked  — Team Orienteering Problem, PSO solver.
    2. Max Coverage Growing masked — greedy max-coverage routing with growing
       visited-cell suppression.

    Both use the same sensor/station placement cache (sensor_cache_key points to
    the TOP combo name so the cache file is shared).
    """
    sensor_params = {
    ...
    top_name = f"GaussianBudget{int(budget_millions)}M_TOP"
    return [
        {
            "name":             top_name,
        ...
        },
        {
            ...
            "sensor_params":    sensor_params,
            # Share the sensor placement cache computed for the TOP combo —
            # both strategies use the same stations, only routing differs.
            "sensor_cache_key": top_name,
        },
    ]
    ...
    for combo in strategy_combinations:
        ...
        combo_custom = {
            **base_custom,
            ...
            "burnmap_type":         "static",   # Pyrologix is (1,N,M); strategy tiles 200× internally
        }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Yearly benchmark on California 2021 (100 fires, parallel, TOP masked, 1 min routing cap, Pyrologix sensor placement)."
    )
    parser.add_argument(
        "--budget",
        type=int,
