#!/usr/bin/env python3
"""
Run the greedy-uniform StationMax budget-based sensor placement on the
California 2021 dataset.  Results are saved in the standard sensor_alloc
cache format.
"""
...
from Strategy import SensorPlacementMaxCoverageGaussianTimeMaskedBudgetStationMaxGreedyUniform
...
def main(
    ...
    fixed_drones_per_station: int = 0,
    cost_sensor_millions: float = 0.1,
):
    cell_size_m = 1000
    speed = 600
    coverage_r_m = 2900
    ...
    combo_name = f"GaussianBudget{int(budget_millions)}M_StationMaxGreedyUniform"
    tag_part = f"_{output_tag.strip()}" if (output_tag and output_tag.strip()) else ""
    # Default path (no tag) matches run_benchmark_california2021_yearly.py; use --output-tag
    # when running parallel placement experiments to avoid overwriting the same JSON.
    ...
    custom_params = {
        ...
        "cost_sensor": float(cost_sensor_millions),
        "cost_station": 0.15,
        "cost_drone": 0.05,
    ...
    print("\n" + "=" * 70, flush=True)
    print("  Running SensorPlacementMaxCoverageGaussianTimeMaskedBudgetStationMaxGreedyUniform", flush=True)
    print("=" * 70 + "\n", flush=True)

    strat = SensorPlacementMaxCoverageGaussianTimeMaskedBudgetStationMaxGreedyUniform(auto_params, custom_params)
    ...
    log_data = {
        ...
        "cost_sensor_millions": float(cost_sensor_millions),
        "cost_station_millions": 0.15,
        "cost_drone_millions": 0.05,
    }
    ...
    budget_used = (
        strat.n_ground_sensors * float(cost_sensor_millions)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--warm-start", type=str, default="",
                        help="Path to a previous sensor_alloc JSON to warm-start Gurobi")
    parser.add_argument("--candidate-percentile", type=float, default=None,
                        help="Julia quantile p: keep top (1-p)*100%% cells (0.8→top 20%%, 0.2→top 80%%).")
    parser.add_argument("--output-tag", type=str, default="",
                        help="Optional tag before .json (parallel runs); e.g. ws100M_filt20")
    parser.add_argument("--fixed-drones-per-station", type=int, default=0,
                        help="If >0, every open charging station has exactly this many drones (MIP constraint).")
    parser.add_argument(
        "--cost-sensor",
        type=float,
        default=0.1,
        help="Ground sensor unit cost in millions of USD (default 0.1 = $100k).",
    )
    ...
    main(
        ...
        cost_sensor_millions=args.cost_sensor,
    )
