...
\title{Sparse Split Optimization for Team Orienteering with Giant Tours}
\author{}
\date{}
...
We denote:
\begin{itemize}[nosep]
  \item $n$: length of the permutation (customers + depots).
  \item $m$: number of vehicles.
  \item $k$: number of depot positions in the permutation (typically $k \ll n$).
  \item $L$: total length of all saturated tours computed from depot positions.
\end{itemize}

\section{Baseline Split Procedure (Dense Form)}

The baseline split operates in three phases:
\begin{enumerate}[nosep]
  \item \textbf{Phase 1 (Saturated Tours)}: For each position $i = 1..n$, if $\pi_i$ is a depot, compute the maximal feasible tour starting at $i$, recording its profit $P[i]$, successor position $\text{succ}[i]$, and tour length $\ell[i]$.
  \item \textbf{Phase 2 (DP)}: Compute a DP table $\Gamma[i, j]$ giving the maximum profit from position $i$ using $j$ vehicles.
  \item \textbf{Phase 3 (Backtracking)}: Reconstruct the optimal set of tours from the DP table.
\end{enumerate}

The dense complexity is $O(n)$ for Phase 1, $O(n \times m)$ for Phase 2, and $O(n)$ for Phase 3.

\section{Key Insights for Sparsification}

\begin{observation}[Tour Starts Are Sparse]
...
\begin{observation}[DP Values Propagate Through Non-Depots]
...
\begin{definition}[Depot Positions]
Let $\mathcal{D} = [d_1 < d_2 < \cdots < d_k]$ be the sorted list of depot positions in the permutation. We compute $\mathcal{D}$ from the node-to-position mapping by collecting all depot node positions and sorting them. The cost is $O(k)$ to collect and $O(k \log k)$ to sort.
\end{definition}

\begin{definition}[Sparse Tour Arrays]
For each depot index $t = 1..k$ corresponding to position $d_t$, we store:
\begin{itemize}[nosep]
  \item $P_{\text{sparse}}[t]$: profit of saturated tour starting at $d_t$
  \item $\text{succ}_{\text{sparse}}[t]$: next position after the tour (0 if it reaches the end)
  \item $\ell_{\text{sparse}}[t]$: tour length
\end{itemize}
These arrays are indexed by depot index, not permutation position.
\end{definition}

\section{Phase 1 Optimization: Sparse Saturated Tours}
...
\section{Phase 2 Optimization: Sparse Dynamic Programming}
...
\begin{theorem}[Sparse DP Correctness]
...
\subsection*{Sparse DP Recurrence}
The helper \texttt{lookup\_Gamma\_sparse} performs a binary search for the first depot position at or after a given successor position. A compact recurrence is given in Appendix~A.
...
\section{Phase 3 Optimization: Sparse Backtracking}
...
\textbf{Total} & $O(n \times m)$ & $O(k \log k + L + k \times m \times \log k)$ \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}[nosep]
  \item \textbf{Node-to-position mapping} should be stored per particle to access depot positions in $O(k)$.
  \item \textbf{Binary search} is used to map successor positions to depot indices.
  \item \textbf{Overloads} of the sparse split can compute \texttt{node\_to\_position} on-the-fly when needed for non-particle permutations.
\end{enumerate}

\appendix

\section{Pseudocode Summary}
