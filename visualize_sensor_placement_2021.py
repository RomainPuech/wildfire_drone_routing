...
WFPI_ZIP_DIR = NATURE_DATA_DIR / "2021_Wind-enhanced_Fire_Potential_Index_Forecast_1_DATA"
# Tiny cached artifacts (shipped) so the geo figures need no raster/shapefile:
#   - georef: crop transform + CRS + dims (grid identical across WFPI forecast horizons)
#   - CA boundary: dissolved Census-tract outline (EPSG:4326)
WFPI_GEOREF_JSON = NATURE_DATA_DIR.parent / "wfpi_georef.json"
CA_BOUNDARY_GEOJSON = NATURE_DATA_DIR.parent / "ca_boundary_wgs84.geojson"
...
def get_pyrologix_georef():
    ...
    # Fast path: cached georef (no raster / shapefile needed).
    if WFPI_GEOREF_JSON.is_file():
        try:
            import json as _json
            import rasterio
            from rasterio.crs import CRS as _CRS
            meta = _json.loads(WFPI_GEOREF_JSON.read_text())
            a, b, c, d, e, f = meta["cropped_transform"]
            _PYROLOGIX_GEOREF_CACHE = (
                rasterio.transform.Affine(a, b, c, d, e, f),
                _CRS.from_wkt(meta["crs_wkt"]),
                int(meta["crop_height"]),
                int(meta["crop_width"]),
            )
            return _PYROLOGIX_GEOREF_CACHE
        except Exception:
...
def california_boundary_pixel_paths(
    ...
    if CA_TRACTS_SHP.is_file():
        ca_tracts = gpd.read_file(CA_TRACTS_SHP).to_crs("EPSG:4326")
        ca_tracts["geometry"] = ca_tracts.buffer(0)
        geom = ca_tracts.dissolve().geometry.iloc[0]
    elif CA_BOUNDARY_GEOJSON.is_file():
        geom = gpd.read_file(str(CA_BOUNDARY_GEOJSON)).to_crs("EPSG:4326").geometry.iloc[0]
    else:
        return []
