#!/usr/bin/env python3
"""
Benchmark on California Dataset — Time-Aware Yearly WFPI Burn Map (multi-year)

Parameterized version of run_benchmark_california2021_yearly.py that supports
any dataset year (2021–2024) via --year and a 75M budget tier via --budget 75.

Key differences vs the 2021-only script:
  - --year YYYY          : selects CaliforniaYYYYDataset/ as the scenario source.
  - --dataset-dir DIR    : explicit override for the dataset directory.
  - --placement-json PATH: explicit placement JSON; default always resolves to
                           California2021Dataset/logs/ (placements are year-independent).
  - --budget now includes 75 (20 | 50 | 75 | 100 | 500).
  - --routing NAME       : alias for --strategy (substring match).
  - --instrument-timing  : emit routing_compute_seconds / sim_replay_seconds columns.
  - --limit-fires N      : cap benchmark subset at N scenarios (0 = dry-run friendly).
  - --output-csv PATH    : explicit CSV output path.
  - --max-workers N      : override MAX_WORKERS.
  - --dry-run            : print resolved paths and exit without loading any data.
  - CSV has a new `year` column set to --year.

Pyrologix is used for BOTH sensor/charging placement AND drone routing decisions.
Placement JSONs live permanently in California2021Dataset/logs/ and are reused
unchanged for 2022–2024 benchmarks (no data-leakage: Pyrologix is trained on
2006–2020 data only).
...
Run from the project root:
  python -u run_benchmark_california_yearly.py --year 2021 --budget 20
  python -u run_benchmark_california_yearly.py --year 2022 --budget 75 --routing TOPGrowing
  python -u run_benchmark_california_yearly.py --year 2021 --budget 100 --no-clustering
  python -u run_benchmark_california_yearly.py --sensor-only --budget 100
"""
...
# ── Dataset paths (2021 defaults; overridden dynamically in main()) ────────────
...
# Placement JSONs live permanently in the 2021 log dir and are reused unchanged
# for all years (year-independent placements).
PLACEMENT_2021_LOG_DIR = PROJECT_ROOT / "California2021Dataset" / "logs"

# Default placement filename by budget (261x161 grid, mean pooling).
# 500M uses a different placement class with extra suffix — looked up separately.
_PLACEMENT_FNAME_BY_BUDGET = {
    20:  "sensor_alloc_GaussianBudget20M_StationMaxGreedyUniform_{N}x{M}_mean.json",
    50:  "sensor_alloc_GaussianBudget50M_StationMaxGreedyUniform_{N}x{M}_mean.json",
    75:  "sensor_alloc_GaussianBudget75M_StationMaxGreedyUniform_{N}x{M}_mean.json",
    100: "sensor_alloc_GaussianBudget100M_StationMaxGreedyUniform_{N}x{M}_mean.json",
}
...
# Budget is selected via CLI (--budget 20 | 50 | 75 | 100 | 500). One combo per run.
...
def compute_clusters(charging_locs_opt: list,
    ...
    cluster (no merging regardless of distance).  The fingerprint is the station
    coordinate ``"<x>-<y>"`` so CSV rows carry per-station identity.
...
def _process_scenario_impl(task):
    ...
    instrument_timing    = ctx.get("instrument_timing", False)
    year_val             = ctx.get("year", 2021)
    pd_census            = ctx.get("pd_census", False)
    ...
    status = None
    routing_compute_seconds = 0.0
    sim_replay_seconds      = 0.0
    ...
    if cluster is None:
        ...
        sim_t0 = time.perf_counter() if instrument_timing else 0.0
        results = check_ground_sensor_detection(
            scenario, ground_locs_data, offset, coverage_w
        )
        if instrument_timing:
            sim_replay_seconds = time.perf_counter() - sim_t0
        results["routed"] = False
        status = "sensor_only"

        if pd_census:
            # Sensor-only fires never involve a drone; the deterministic
            # (fixed-sensor) outcome is unaffected by the drone detection
            # probability and is passed through as a deterministic terminal.
            results["pd_drone_passes"] = []
            if results.get("device") in ("ground sensor", "charging station") \
                    and results.get("delta_t", -1) >= 0:
                results["pd_det_terminal"] = [int(results["delta_t"]), results["device"]]
            else:
                results["pd_det_terminal"] = None
            results["pd_offset"] = offset
            results["pd_cluster_fp"] = "none"
        ...
        if not _routing_log_has(routing_log_file, max_substeps):
            ...
            routing_t0 = time.perf_counter() if instrument_timing else 0.0
            ...
            if instrument_timing:
                routing_compute_seconds = time.perf_counter() - routing_t0
        ...
        if pd_census:
            passes   = census.get("drone_passes", [])
            terminal = census.get("det_terminal")
            results["pd_drone_passes"] = passes
            results["pd_det_terminal"] = terminal
            results["pd_offset"] = offset
            results["pd_cluster_fp"] = cluster["fingerprint"]
            # In census mode run_simulation does not break on drone detection, so
            # reconstruct the deterministic (Pd=1) outcome from the census: the
            # earliest of the first drone-pass hour and the fixed-sensor terminal.
            cand = []
            if passes:
                cand.append((int(passes[0][0]), "drone"))
            if terminal is not None:
                cand.append((int(terminal[0]), terminal[1]))
            if cand:
                h_b, dev_b = min(cand, key=lambda x: x[0])
                results["delta_t"] = h_b
                results["device"]  = dev_b
    ...
    return {
        ...
        "year":           year_val,
        "routing_compute_seconds": routing_compute_seconds,
        "sim_replay_seconds":      sim_replay_seconds,
        "_status":        status,
        **results,
    }
...
def _ground_detection_cells(ground_locs_data: list, coverage_w: int,
    ...
    A ground sensor is placed/stored as the data-scale **center** cell of its
    5x5 operational cell. A fire counts as detected by a ground sensor whenever
    its ignition lies in the *same operational cell* as the sensor -- the same
...
def run_simulation(scenario: np.ndarray,
                   ...
                   census_pass_log: dict | None = None) -> dict:
    ...
    *fire_start_idx* is the first data step with any burning cell in *scenario*.
    ...
    If *census_pass_log* (a dict) is provided, the function additionally runs in
    a non-destructive "detection census" mode used by the Pd-sensitivity replay:
    instead of stopping at the first drone observation of the fire, it records
    *every* per-hour drone detection opportunity over the full detection window.
    This does not alter the returned deterministic (Pd=1) ``delta_t``/``device``.
    The dict is populated with:

      * ``drone_passes``  : list of ``[hour, n_hits]`` where ``hour`` is the data
        step (== Pd=1 ``delta_t`` value) and ``n_hits`` is the number of
      ...
      * ``det_terminal``  : ``[hour, device]`` for the first hour a *fixed* sensor
        (ground/charging) covers the fire (deterministic backstop), else ``None``.
      * ``starting_time`` : the scenario offset (echoed for convenience).
      * ``window_hi``     : the exclusive upper bound of the scored time window.

    Fixed-sensor (ground/charging) detection is treated as deterministic and
    terminates the census at that hour, mirroring the deterministic replay where
    those sensors are checked before drones each hour.
    """
    ...
    if census_pass_log is not None:
        census_pass_log["drone_passes"] = []
        census_pass_log["det_terminal"] = None
        census_pass_log["starting_time"] = starting_time
        census_pass_log["window_hi"] = loop_hi
    ...
    for time_step in range(-starting_time, loop_hi):
        ...
        if time_step >= 0:
            ...
            if rows_g and (grid[rows_g, cols_g] == 1).any():
                ...
                if census_pass_log is not None:
                    census_pass_log["det_terminal"] = [time_step, "ground sensor"]
                break
            ...
            if rows_c and (grid[rows_c, cols_c] == 1).any():
                ...
                fire_size_percentage = fire_size_cells / (grid.shape[0] * grid.shape[1]) * 100
                if census_pass_log is not None:
                    census_pass_log["det_terminal"] = [time_step, "charging station"]
                break
        ...
        if census_pass_log is not None and time_step >= 0 and hour_drone_hits > 0:
            census_pass_log["drone_passes"].append([time_step, hour_drone_hits])
...
def main(budget_millions: float = 20.0, clustering_enabled: bool = True,
         ...
         *,
         ...
         placement_json_override: str | None = None,
         instrument_timing: bool = False,
         limit_fires: int | None = None,
         fire_names_file: str | None = None,
         ...
         emit_pass_catalog: str | None = None):

    # ── Resolve dataset and log directories ───────────────────────────────────
    year = int(year)
    if dataset_dir is not None:
        eff_dataset_dir = PROJECT_ROOT / dataset_dir
    else:
        eff_dataset_dir = PROJECT_ROOT / f"California{year}Dataset"

    eff_placement_map  = eff_dataset_dir / "static_risk_pyrologix.npy"
    eff_mask_path      = eff_dataset_dir / "mask.npy"
    eff_config_path    = eff_dataset_dir / f"config_california_{year}.json"
    eff_scenarii_dir   = eff_dataset_dir / "scenarii"

    # Routing cache and sensor placement JSONs are year-independent:
    # always look in California2021Dataset/logs/.
    eff_log_dir = PLACEMENT_2021_LOG_DIR

    eff_log_dir.mkdir(exist_ok=True)
    TMP_DIR.mkdir(exist_ok=True)

    # Effective max workers
    eff_max_workers = max_workers_override if max_workers_override is not None else MAX_WORKERS
    ...
    print(f"Dataset dir: {eff_dataset_dir}", flush=True)
    print(f"Routing/placement log dir: {eff_log_dir}", flush=True)
    print(f"Station clustering: {'enabled' if clustering_enabled else 'DISABLED (one cluster per station)'}", flush=True)
    if instrument_timing:
        print(f"Timing instrumentation: ON (routing_compute_seconds + sim_replay_seconds)", flush=True)

    # ── Static resources ──────────────────────────────────────────────────────
    print("Loading Pyrologix placement map ...", flush=True)
    avg_map = np.load(str(eff_placement_map))

    print("Loading mask ...", flush=True)
    mask = np.load(str(eff_mask_path))
    H, W = mask.shape

    print("Loading config ...", flush=True)
    with open(eff_config_path) as f:
        config = json.load(f)
    ...
    rescaled_mask_path = str(eff_mask_path).replace(".npy", suffix)
    ...
    rescaled_avg_path     = str(eff_placement_map).replace(".npy", f"_{SENSOR_POOLING}{suffix}")
    rescaled_routing_path = str(eff_placement_map).replace(".npy", f"_{SENSOR_POOLING}_routing{suffix}")
    ...
    # ── Valid scenario list ───────────────────────────────────────────────────
    all_scenario_files = sorted(eff_scenarii_dir.glob("*.npy"))
    valid_scenarios = [
    ...
    # ── Fire names filter (--fire-names-file) ─────────────────────────────────
    if fire_names_file is not None:
        allowed_names = set(Path(fire_names_file).read_text().splitlines())
        allowed_names = {n.strip() for n in allowed_names if n.strip()}
        valid_scenarios = [
            sf for sf in valid_scenarios
            if sf.stem.replace("_scenario1", "") in allowed_names
        ]
        print(
            f"Fire names filter: {len(valid_scenarios)} scenarios kept "
            f"(from {len(allowed_names)} names in {fire_names_file})",
            flush=True,
        )

    # ── Random subset selection ───────────────────────────────────────────────
    # --limit-fires overrides BENCHMARK_SUBSET_SIZE (0 = no scenarios, useful for dry-run)
    eff_subset_size = BENCHMARK_SUBSET_SIZE if limit_fires is None else int(limit_fires)
    if eff_subset_size == 0:
        benchmark_scenarios = []
        print("limit-fires=0: no scenarios selected (smoke-test / dry-run mode).", flush=True)
    elif eff_subset_size >= len(valid_scenarios):
        benchmark_scenarios = valid_scenarios
        print(
            f"Using all {len(benchmark_scenarios)} valid scenarios.",
            flush=True,
        )
    else:
        rng = np.random.default_rng(RANDOM_SEED)
        subset_idx = np.sort(rng.choice(len(valid_scenarios), size=eff_subset_size, replace=False))
        benchmark_scenarios = [valid_scenarios[i] for i in subset_idx]
        print(
            f"Random subset: {eff_subset_size} scenarios (seed={RANDOM_SEED})\n",
            flush=True,
        )
    ...
    for combo in strategy_combinations:
        ...
        # Priority: placement_json_override > sensor_placement_file > default from 2021 logs
        if placement_json_override is not None:
            sensor_log_path = placement_json_override
        elif sensor_placement_file is not None:
            sensor_log_path = sensor_placement_file
        else:
            # Default: always look in California2021Dataset/logs/ (year-independent)
            bm = int(budget_millions)
            if bm in _PLACEMENT_FNAME_BY_BUDGET:
                fname = _PLACEMENT_FNAME_BY_BUDGET[bm].format(N=rescaled_N, M=rescaled_M)
            else:
                # Fallback using the same pattern as the original SC code
                fname = f"sensor_alloc_{sensor_cache_name}_{rescaled_N}x{rescaled_M}_{SENSOR_POOLING}.json"
            sensor_log_path = str(eff_log_dir / fname)
            ...
            if not matching:
                ...
                for cluster in matching:
                    routing_log_file = _routing_log_path(
                        str(eff_log_dir), base_routing_cls, oh, rs,
        ...
        shared_ctx = {
            ...
            "year":                 year,
            "pd_census":            emit_pass_catalog is not None,
        }
        ...
        for wave in range(n_waves):
            ...
            if not wave_tasks:
                continue
            n_workers = min(eff_max_workers, len(wave_tasks))
            print(
    ...
    # ── Pd-sensitivity pass catalog (per-task, pre-aggregation) ──────────────
    # Emit one JSONL record per (strategy, scenario, cluster) task BEFORE the
    # best-row aggregation below, so the Pd-replay aggregator can pool drone
    # detection opportunities across all stations that can reach a fire. The
    # heavy pd_* fields are then stripped from the in-memory rows so the CSV
    # output is byte-identical to a normal (non-census) run.
    if emit_pass_catalog is not None and all_results:
        catalog_path = Path(emit_pass_catalog)
        catalog_path.parent.mkdir(parents=True, exist_ok=True)
        with open(catalog_path, "w") as fcat:
            for r in all_results:
                rec = {
                    "strategy_combo": r.get("strategy_combo"),
                    "scenario_name":  r.get("scenario_name"),
                    "year":           r.get("year"),
                    "offset":         r.get("pd_offset", r.get("offset")),
                    "cluster_fp":     r.get("pd_cluster_fp", r.get("cluster")),
                    "routed":         r.get("routed", False),
                    "baseline_delta_t": r.get("delta_t"),
                    "baseline_device":  r.get("device"),
                    "drone_passes":   r.get("pd_drone_passes", []),
                    "det_terminal":   r.get("pd_det_terminal"),
                }
                fcat.write(json.dumps(rec) + "\n")
        # Strip pd_* keys so downstream DataFrame/CSV is unaffected.
        for r in all_results:
            for k in ("pd_drone_passes", "pd_det_terminal", "pd_offset", "pd_cluster_fp"):
                r.pop(k, None)
        print(f"\nPass catalog written to: {catalog_path}  ({len(all_results)} task records)",
              flush=True)
    ...
    if all_results:
        ...
        if output_csv is not None:
            ...
            csv_path = str(results_dir / f"benchmark_results_yearly_{int(budget_millions)}M_{year}_{timestamp}.csv")
...
def run_sensor_placement_only(budget_millions: float, time_limit_seconds: float = 600.0,
                               year: int = 2021, dataset_dir: str | None = None):
    ...
    sensor_log_path = str(eff_log_dir / f"sensor_alloc_{sensor_only_cache_name}_{rescaled_N}x{rescaled_M}_{SENSOR_POOLING}.json")
    sensor_custom = {
        **base_custom,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Yearly benchmark on California dataset (multi-year, parallel, TOP masked, "
            "2 min routing cap per re-eval, Pyrologix sensor placement). "
            "Use --year to select the dataset year (default: 2021). "
            "Placement JSONs always come from California2021Dataset/logs/ (year-independent)."
        )
    )
    parser.add_argument(
        "--year",
        ...
        default=2021,
        metavar="YYYY",
        help="Dataset year (default: 2021). Selects California<YEAR>Dataset/ unless --dataset-dir is set.",
    )
    parser.add_argument(
        "--budget",
        type=int,
        choices=[20, 50, 75, 100, 500],
        default=20,
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="DIR",
        help="Override dataset directory (default: California<YEAR>Dataset relative to project root).",
    )
    parser.add_argument(
        "--placement-json",
        type=str,
        ...
        metavar="PATH",
        help=(
            "Explicit sensor placement JSON path. "
            "Default: California2021Dataset/logs/sensor_alloc_GaussianBudget<B>M_StationMaxGreedyUniform_<N>x<M>_mean.json "
            "(always uses 2021 placement regardless of --year)."
        ),
    )
    parser.add_argument(
        "--sensor-only",
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="NAME",
        help="Alias for --strategy: run only strategies whose name contains NAME.",
    )
    parser.add_argument(
        "--reevaluation-step",
        type=int,
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="PATH",
        help="Override sensor placement JSON path (legacy alias; prefer --placement-json).",
    )
    parser.add_argument(
        "--instrument-timing",
        action="store_true",
        help=(
            "Add routing_compute_seconds and sim_replay_seconds columns to output CSV. "
            "routing_compute_seconds=0 when routing log was already cached."
        ),
    )
    parser.add_argument(
        "--limit-fires",
        ...
        default=None,
        metavar="N",
        help="Cap the benchmark subset at N scenarios (0 = no fires, useful for smoke-testing setup).",
    )
    parser.add_argument(
        "--fire-names-file",
        type=str,
        ...
        metavar="PATH",
        help=(
            "Path to a text file with one scenario_name per line. "
            "When set, only those scenarios are run (applied before --limit-fires)."
        ),
    )
    parser.add_argument(
        "--output-csv",
        ...
        metavar="PATH",
        help=(
            "Output CSV path (default: results/benchmark_results_yearly_<B>M_<YEAR>_<timestamp>.csv). "
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="N",
        help=f"Override number of parallel worker processes (default: {MAX_WORKERS} from cpu_count/SLURM).",
    )
    parser.add_argument(
        "--emit-pass-catalog",
        type=str,
        ...
        metavar="PATH",
        help=(
            "Pd-sensitivity replay: write a JSONL pass catalog (one record per "
            "strategy x scenario x cluster) capturing every per-hour drone "
            "detection opportunity over the scoring window, plus the fixed-sensor "
            "terminal. Used with the cached routing logs; does not re-optimize "
            "routing and leaves the output CSV unchanged. Feed the JSONL to "
            "scripts/pd_sensitivity_aggregate.py to sweep the per-pass detection "
            "probability."
        ),
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print resolved paths and configuration, then exit without loading any data files.",
    )
    args = parser.parse_args()

    # --routing is an alias for --strategy; --strategy takes precedence if both given
    effective_strategy = args.strategy or args.routing

    # --placement-json takes precedence over --sensor-placement-file
    effective_placement_json = args.placement_json or args.sensor_placement_file

    if args.dry_run:
        year_v = args.year
        ds_dir = (PROJECT_ROOT / args.dataset_dir) if args.dataset_dir else (PROJECT_ROOT / f"California{year_v}Dataset")
        print(f"[dry-run] year={year_v}", flush=True)
        print(f"[dry-run] budget={args.budget}M", flush=True)
        print(f"[dry-run] dataset_dir={ds_dir}", flush=True)
        print(f"[dry-run] placement_log_dir={PLACEMENT_2021_LOG_DIR}", flush=True)
        print(f"[dry-run] placement_json={effective_placement_json or '(default from budget)'}", flush=True)
        print(f"[dry-run] no_clustering={args.no_clustering}", flush=True)
        print(f"[dry-run] strategy_filter={effective_strategy!r}", flush=True)
        print(f"[dry-run] instrument_timing={args.instrument_timing}", flush=True)
        print(f"[dry-run] limit_fires={args.limit_fires}", flush=True)
        print(f"[dry-run] output_csv={args.output_csv or '(auto)'}", flush=True)
        print("[dry-run] Exiting without loading data.", flush=True)
        sys.exit(0)
    ...
    main(
        ...
        strategy_filter=effective_strategy,
        year=args.year,
        dataset_dir=args.dataset_dir,
        placement_json_override=effective_placement_json,
        instrument_timing=args.instrument_timing,
        limit_fires=args.limit_fires,
        fire_names_file=args.fire_names_file,
        ...
        sensor_placement_file=None,  # handled by placement_json_override above
        emit_pass_catalog=args.emit_pass_catalog,
    )
