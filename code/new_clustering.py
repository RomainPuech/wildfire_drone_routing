...
class ClusteredDroneStrategyWrapped:
    base_cls = None  # <-- will be set dynamically via the factory

    def __init__(self, automatic_initialization_parameters: dict, custom_initialization_parameters: dict):
        BaseStrategy = self.base_cls
        self.strategy_instances = []

        total_drones = automatic_initialization_parameters["n_drones"]
        self.clusters = self.find_clusters(automatic_initialization_parameters["charging_stations_locations"],
                                           automatic_initialization_parameters["max_battery_time"])
        drone_battery = automatic_initialization_parameters["max_battery_time"]
        drone_transmission_range = automatic_initialization_parameters["transmission_range"]
        ...
        for cid, stations in enumerate(self.clusters):
            ...
            auto_params_cluster = automatic_initialization_parameters.copy()
            auto_params_cluster["charging_stations_locations"] = stations
            auto_params_cluster["n_drones"] = drones_per_cluster[cid]
            auto_params_cluster["ground_sensor_locations"] = this_cluster_sensor_locations
            auto_params_cluster["n_charging_stations"] = len(stations)

            strat = BaseStrategy(auto_params_cluster, custom_initialization_parameters)
            self.strategy_instances.append(strat)
    ...
    def find_clusters(self, charging_stations, drone_battery):
        ...
        for i in range(n):
            for j in range(i+1, n):
                if math.hypot(charging_stations[i][0] - charging_stations[j][0],
                              charging_stations[i][1] - charging_stations[j][1]) <= radius:
                    adj[i].append(j)
                    adj[j].append(i)
...
def get_wrapped_clustering_strategy(BaseStrategy):
    name = f"Clustered{BaseStrategy.__name__}"
    ...
    Wrapped = type(
        ...
        {
            'base_cls': BaseStrategy,
            '__module__': 'wrappers'  # ensures pickle finds it
        }
    )
