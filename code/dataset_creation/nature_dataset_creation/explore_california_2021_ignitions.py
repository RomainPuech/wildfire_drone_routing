#!/usr/bin/env python3
"""
Explore and filter California 2021 ignition points from USFS dataset.

Filters:
  - State: California only (UNIQFIREID starts with '2021-CA')
...
Outputs:
  - Plot saved to report/california_2021_ignition_points.png
  - Summary saved to report/california_2021_ignition_points.md
"""
...
PLOT_OUT      = os.path.join(REPORT_DIR, "california_2021_ignition_points.png")
MARKDOWN_OUT  = os.path.join(REPORT_DIR, "california_2021_ignition_points.md")
...
# California 2021 wildfires
df_ca_2021 = df[
    (df["FIREYEAR"] == 2021) &
    (df["UNIQFIREID"].str.startswith("2021-CA", na=False)) &
...
print(f"  CA 2021 WF fires (pre-urban filter): {len(df_ca_2021):,}")

# Parse discovery datetime
df_ca_2021["discovery_dt"] = pd.to_datetime(
    df_ca_2021["DISCOVERYDATETIME"], errors="coerce", utc=True
)
df_ca_2021 = df_ca_2021[df_ca_2021["discovery_dt"].notna()].copy()
print(f"  CA 2021 WF fires with valid datetime: {len(df_ca_2021):,}")
...
all_fire_gdf = gpd.GeoDataFrame(
    df_ca_2021,
    geometry=[Point(lon, lat) for lon, lat in zip(df_ca_2021["LONGDD83"], df_ca_2021["LATDD83"])],
    crs="EPSG:4326"
)
...
ax.set_title(
    f"California 2021 — Stage-1 Filter Results\n"
...
markdown = f"""# California 2021 Non-Urban Wildfire Ignition Points
...
| CA 2021 WF fires (raw) | {len(df_ca_2021):,} |
...
print("\nDone!")
print(f"  Non-urban CA 2021 fires: {n_nonurban}")
