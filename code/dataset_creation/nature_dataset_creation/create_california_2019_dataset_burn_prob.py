#!/usr/bin/env python3
"""
Create California 2020 Wildfire Dataset using Burn Probability Map (FSim)

This script creates a dataset with:
- One statewide layout using static burn probability map (FSim)
...
OUTPUT_DIR = os.path.join(BASE_DIR, "../../../California2019Dataset_BurnProb")
...
BURN_PROB_PATH = os.path.join(DATA_DIR, "RDS-2016-0034-2/Data/I_FSim_CONUS_LF2014_270m/CONUS_iBP.tif")
...
def create_california_mask(burn_prob_data, burn_prob_transform, burn_prob_crs, urban_gdf=None, california_gdf=None):
    """
    Create a mask for California from burn probability data.
    ...
    if california_gdf is not None:
        burn_prob_data_3d = burn_prob_data[np.newaxis, :, :] if burn_prob_data.ndim == 2 else burn_prob_data
        cropped_data, new_transform, crop_bounds = crop_to_california(
            burn_prob_data_3d, burn_prob_transform, burn_prob_crs, california_gdf
        )
        # Ensure 2D
        if cropped_data.ndim == 3:
            burn_prob_data = cropped_data[0]
        else:
            burn_prob_data = cropped_data
        burn_prob_transform = new_transform
    else:
        crop_bounds = (0, burn_prob_data.shape[0], 0, burn_prob_data.shape[1])
        new_transform = burn_prob_transform
    
    # Start with validity mask (not NaN, not negative) - ensure 2D
    if burn_prob_data.ndim == 3:
        burn_prob_data_2d = burn_prob_data[0]
    else:
        burn_prob_data_2d = burn_prob_data
    mask = np.ones_like(burn_prob_data_2d, dtype=np.float32)
    mask[np.isnan(burn_prob_data_2d)] = 0
    mask[burn_prob_data_2d < 0] = 0
    
    # Mask areas outside California boundary (if provided)
    if california_gdf is not None:
        california_raster_crs = california_gdf.to_crs(burn_prob_crs)
        ...
        # Use 2D dimensions
        height, width = burn_prob_data_2d.shape
        left = burn_prob_transform[2]
        top = burn_prob_transform[5]
        right = left + width * burn_prob_transform[0]
        bottom = top + height * burn_prob_transform[4]
        
        # Rasterize California boundary
        california_mask_raster = rasterize(
            shapes,
            out_shape=(height, width),
            transform=burn_prob_transform,
            fill=0,
            default_value=1,
            dtype=np.uint8
        )
    ...
    if urban_gdf is not None:
        urban_raster_crs = urban_gdf.to_crs(burn_prob_crs)
...
def main():
    print("=" * 80)
    print("California 2019 Wildfire Dataset Creation (Burn Probability Map - FSim)")
    print("=" * 80)
    ...
    # Get all CA 2019 fires with valid dates
    cursor.execute("""
    ...
    print(f"Found {len(all_fires)} CA 2019 fires with valid dates")
    ...
    # Step 3: Load burn probability map
    print("\n[3/7] Loading burn probability map...")
    with rasterio.open(BURN_PROB_PATH) as src:
        burn_prob_data = src.read(1)
        burn_prob_meta = src.meta.copy()
        burn_prob_transform = src.transform
        burn_prob_crs = src.crs
    
    # Handle NoData values (very large negative number)
    nodata_val = burn_prob_meta.get('nodata', -3.4028230607370965e+38)
    burn_prob_data = np.where(burn_prob_data == nodata_val, np.nan, burn_prob_data)
    
    print(f"Burn probability map dimensions: {burn_prob_data.shape}")
    print(f"CRS: {burn_prob_crs}")
    print(f"Resolution: {src.res}")
    print(f"Data range: [{np.nanmin(burn_prob_data):.6f}, {np.nanmax(burn_prob_data):.6f}]")
    ...
    california_mask, mask_transform, crop_bounds = create_california_mask(
        burn_prob_data, burn_prob_transform, burn_prob_crs, urban_gdf, california_boundary_gdf
    )
    ...
    # Step 5: Crop and save burn probability map
    print("\n[5/7] Cropping and saving burn probability map...")
    burn_prob_data_3d = burn_prob_data[np.newaxis, :, :] if burn_prob_data.ndim == 2 else burn_prob_data
    cropped_burn_prob, cropped_transform, _ = crop_to_california(
        burn_prob_data_3d, burn_prob_transform, burn_prob_crs, california_boundary_gdf
    )
    
    # Save cropped burn probability map
    burn_prob_path = os.path.join(OUTPUT_DIR, "static_risk_burn_prob.npy")
    burn_prob_3d = cropped_burn_prob[np.newaxis, :, :].astype(np.float32) if cropped_burn_prob.ndim == 2 else cropped_burn_prob.astype(np.float32)
    np.save(burn_prob_path, burn_prob_3d)
    print(f"Saved burn probability map to {burn_prob_path}")
    print(f"  Shape: {burn_prob_3d.shape}")
    
    # Update transform and data for coordinate conversion
    burn_prob_transform = cropped_transform
    # Ensure 2D
    if cropped_burn_prob.ndim == 3:
        burn_prob_data = cropped_burn_prob[0]
    else:
        burn_prob_data = cropped_burn_prob
    grid_height, grid_width = burn_prob_data.shape
    ...
    if isinstance(burn_prob_crs, str):
        burn_prob_crs_obj = rasterio.crs.CRS.from_string(burn_prob_crs)
    else:
        burn_prob_crs_obj = burn_prob_crs
    transformer = Transformer.from_crs("EPSG:4326", burn_prob_crs_obj, always_xy=True)
    ...
    for fire in tqdm(non_urban_fires, desc="Processing fires"):
        try:
            ...
            row, col = rasterio.transform.rowcol(burn_prob_transform, x, y)
    ...
    config_path = os.path.join(OUTPUT_DIR, "config_california_2019_burn_prob.json")
    ...
    summary = {
        'dataset_name': 'California2019_BurnProb',
        ...
        'crs': str(burn_prob_crs),
        'resolution_meters': 270.0,
        'date_range': {
