#!/usr/bin/env python3
"""
Create California 2020 Wildfire Dataset

This script creates a dataset with:
- One statewide layout using WFPI Day 2 forecast maps
...
OUTPUT_DIR = os.path.join(BASE_DIR, "../../../California2020Dataset_LargeFires")
...
WFPI_DATA_DIR = os.path.join(DATA_DIR, "2020_Wind-enhanced_Fire_Potential_Index_Forecast_2_DATA")
...
def get_wfpi_date_for_fire(fire_date):
    ...
    We use Day 2 forecast from the day BEFORE the fire (to avoid data contamination).
    """
...
def find_wfpi_file(target_date):
    ...
    zip_pattern = f"wfpi-forecast-2_data_{target_date}_{target_date}.zip"
...
def main():
    print("=" * 80)
    print("California 2020 Wildfire Dataset Creation (Large Fires Only, >= 100 acres)")
    print("=" * 80)
    ...
    for fire in all_fires:
        ...
        if parsed_date:
            # Skip fires on January 1st (would need Dec 31, 2019 WFPI data)
    ...
    if skipped_jan1 > 0:
        print(f"Skipped {skipped_jan1} fires on January 1st (no WFPI data from Dec 31, 2019)")
    ...
    config_path = os.path.join(OUTPUT_DIR, "config_california_2020_large_fires.json")
    ...
    summary = {
        'dataset_name': 'California2020_LargeFires',
