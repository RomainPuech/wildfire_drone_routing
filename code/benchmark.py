...
def run_drone_routing_strategy(drone_routing_strategy:DroneRoutingStrategy, sensor_placement_strategy:SensorPlacementStrategy, T:int, canonical_scenario:np.ndarray, automatic_initialization_parameters_function:callable, custom_initialization_parameters_function:callable, custom_step_parameters_function:callable, input_dir:str='', simulation_parameters:dict={}, file_format:str="npy", starting_time:int=1, scenario_name:str=""):
    ...
    fire_detected = False
    for time_step in tqdm.tqdm(range(-starting_time,min(12,len(canonical_scenario)))): # if fire is not detected in 24 time steps, we stop the simulation
    ...
    total_time_steps = t_found  # This is the number of time steps computed
    # Return execution time per step (or total time if 0 steps)
    return total_time / max(1, total_time_steps)
...
def run_benchmark_scenarii_sequential(input_dir, sensor_placement_strategy:SensorPlacementStrategy, drone_routing_strategy:DroneRoutingStrategy, custom_initialization_parameters_function:callable, custom_step_parameters_function:callable, starting_time:int=0, max_n_scenarii:int=None, file_format="npy", simulation_parameters:dict={}, config:dict={}, precomputing_time:float=0, experiment_name:str=''):
    """
    ...
    for file in tqdm.tqdm(iterable, total = N_SCENARII):
        ...
        if precomputing_time > 0:
            # Replace execution time with precomputation time per step
            total_execution_times.append(precomputing_time)
        else:
            total_execution_times.append(results["avg_execution_time"])

        # Use precomputation time per step if available, otherwise use individual execution time
        execution_time_to_record = precomputing_time if precomputing_time > 0 else results["avg_execution_time"]
        ...
        per_scenario_results.append({
            ...
            "delta_t": delta_t,
            "device": device,
            "execution_time": execution_time_to_record,
...
def benchmark_on_sim2real_dataset_precompute_parallel(dataset_folder_name, ground_placement_strategy, drone_routing_strategy, custom_initialization_parameters_function, custom_step_parameters_function, max_n_scenarii=None, starting_time=0, max_n_layouts=None, simulation_parameters:dict={}, skip_folder_names:list=[], selected_layout_names:list=[], file_format="npy", config_file:str='', experiment_name:str=''):
    ...
    all_metrics = {}
    max_workers = min(multiprocessing.cpu_count(), 15)
