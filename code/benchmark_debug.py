...
def custom_init_fn(input_dir, scenario_name: str = None, canonical_scenario_name: str = None):
    layout_dir = os.path.abspath(os.path.join(input_dir, ".."))
    recompute = scenario_name == canonical_scenario_name
    print(f"[INIT_FN] scenario={scenario_name}, canonical={canonical_scenario_name}, recompute={recompute}")
    return {
...
if __name__ == "__main__":
    ...
    from code.benchmark_updated import benchmark_on_sim2real_dataset_precompute
    from Strategy import RandomSensorPlacementStrategy, DroneRoutingMaxCoverageResetStatic

    SensorStrat = wrap_log_sensor_strategy(RandomSensorPlacementStrategy)
    DroneStrat = wrap_log_drone_strategy(get_wrapped_clustering_strategy(DroneRoutingMaxCoverageResetStatic))

    start_seq = time.time()
    all_metrics = benchmark_on_sim2real_dataset_precompute(
        dataset_folder_name="../DroneBench/",
        ground_placement_strategy=SensorStrat,
        drone_routing_strategy=DroneStrat,
        custom_initialization_parameters_function=custom_init_fn,
        custom_step_parameters_function=step_fn,
        max_n_scenarii=None,
        starting_time=0,
        max_n_layouts=None,  # Keep small for sequential test
        simulation_parameters=simulation_parameters,
        skip_folder_names=[],
        selected_layout_names=["0019_01316"],
        file_format="jpg",
        config_file='',
