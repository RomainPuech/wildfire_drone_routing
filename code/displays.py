...
def _colorbar_inset_top_right(
    ...
    *,
    ...
    # Use the native Axes.inset_axes with an explicit fractional rectangle in
    # axes coordinates. The axes_grid1 inset_locator (bbox_to_anchor + loc) is
    # evaluated lazily at draw time and is unstable under bbox_inches="tight" on
    # vector backends (PDF/SVG): the colorbar frame/ticks land top-right while the
    # colored solids are mislocated to the bottom-left. A transform-based inset is
    # stable across the Agg and PDF backends.
    cb_w, cb_h = 0.045, 0.36  # fractions of the parent axes
    cb_x = 1.0 - cb_w - 0.11  # leave room for ticks + vertical label to the right
    cb_y = 1.0 - cb_h - 0.045
    cax = ax.inset_axes([cb_x, cb_y, cb_w, cb_h], transform=ax.transAxes)
