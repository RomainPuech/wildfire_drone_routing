...
def run_one_drone_strategy(sensor_strategy, drone_strategy, custom_initialization_parameters_function, experiment_name):
    ...
    benchmark_on_sim2real_dataset_precompute(
        ...
        selected_layout_names= ["0264_02426"],
        file_format="npy",
    ...
    try:
        layout_name = get_latest_layout_name_from_logs(dataset_folder_name)
        generate_video_from_latest_run(experiment_name, layout_name, dataset_folder_name)
    except Exception as e:
        print(f"[!] Error generating video for {experiment_name}: {e}")
...
def run_all_drone_strategies(sensor_strategy, ss_prefix, bm_prefix):
    ...
    run_one_drone_strategy(sensor_strategy, wrap_log_drone_strategy(get_wrapped_clustering_strategy(DroneRoutingMaxCoverageResetStatic)), custom_initialization_parameters_function, f"{ss_prefix}Ug{bm_prefix}")
